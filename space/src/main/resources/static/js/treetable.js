layui.define(["form"], function(exports) {
    ...
    tree.prototype.template = function(data) {
        ...
        o.each(data, function(index, item) {
            var hide_class = 'class="' + (item.parentId == 0 || item.parentId == t.cache(item.parentId) || t.c.is_open ? "" : "hide") + '"',
                tr = '<tr data-id="' + item.id + '" data-parentId="' + item.parentId + '" ' + hide_class + ">" +
                    (t.c.is_checkbox ? '<td><div><input type="checkbox" lay-skin="primary" lay-filter="lay-t"></div></td>' : "");
            level[item.id] = item.parentId > 0 ? (level[item.parentId] + 1) : 0;
            o.each(t.c.cols, function(idx, obj) {
        ...
        return '<thead><tr data-id="0" data-parentId="-1">' + thead + "</tr></thead><tbody>" + tbody + "</tbody>"
    };
    tree.prototype.render = function(e) {
        ...
        o.each(t.c.data, function(index, item) {
            if(!t.c.childs[item.parentId]) {
                t.c.childs[item.parentId] = []
            }
            t.c.childs[item.parentId][item.id] = t.c.new_data[item.id] = data[item.id] = item
        });
        ...
        o(t.c.elem).html(template).on("click", "td", function() {
            var id = o(this).parents("tr").data("id"),
                parentId = o(this).parents("tr").data("parentId"),
                status = o(t.c.elem).find("tr[data-parentId=" + id + "]").is(":visible"),
            ...
            if(dt.length) {
                if(status) {
                    ...
                    o(t.c.elem).find("tr[data-parentId=" + id + "]").removeClass('hide');
                    t.cache(id, true);
    ...
    tree.prototype.tree = function(lists, parentId, data) {
        var t = this;
        if(lists[parentId]) {
            data.push(lists[parentId]);
    ...
    tree.prototype.all = function(type) {
        var t = this;
        if(type == "up") {
            o(t.c.elem).find("tr[data-parentId=0]").each(function() {
                var id = o(this).data("id");
                t.hide(id);
                o(this).find("." + t.c.icon_class).html(t.c.icon_val.open)
            })
